...
*------------------------------------------*/
add_action('init', 'codex_custom_init_product_category');
function codex_custom_init_product_category()
{
   $labels = array(
   ...
   register_taxonomy( 'product-cat', 'our_products', $args );
}

/*-------------------------------------------*
All Crops Post
*------------------------------------------*/
add_action('init', 'home_crop');
function home_crop()
{
    $labels = array(
                    'name'              => _x('Crop', 'post type general name'),
                    'singular_name'     => _x('Crop', 'post type singular name'),
                    'add_new'           => _x('Add New', 'Crop'),
                    'add_new_item'      => __('Add New Crop'),
                    'edit_item'         => __('Edit Crop'),
                    'new_item'          => __('New Crop'),
                    'all_items'         => __('All Crops'),
                    'view_item'         => __('View Crop'),
                    'search_items'      => __('Search Crop'),
                    'not_found'         =>  __('No Crop found'),
                    'not_found_in_trash'=> __('No Crop found in Trash'),
                    'parent_item_colon' => '',
                    'menu_name'         => 'Crops'
                  );
    $args = array(
...
*      Crop-category 
*------------------------------------------*/
add_action('init', 'codex_custom_init_crop_category');
function codex_custom_init_crop_category()
{
   $labels = array(
       'name'              => _x( 'Crop', 'taxonomy general name', 'textdomain' ),
       'singular_name'     => _x( 'Crop', 'taxonomy singular name', 'textdomain' ),
       'search_items'      => __( 'Search Crop', 'textdomain' ),
       'all_items'         => __( 'All Crops', 'textdomain' ),
       'parent_item'       => __( 'Parent Crop', 'textdomain' ),
       'parent_item_colon' => __( 'Parent Crop:', 'textdomain' ),
       'edit_item'         => __( 'Edit Crop', 'textdomain' ),
       'update_item'       => __( 'Update Crop', 'textdomain' ),
       'add_new_item'      => __( 'Add New Crop Category', 'textdomain' ),
       'new_item_name'     => __( 'New Crop Name', 'textdomain' ),
       'menu_name'         => __( 'Crop Category', 'textdomain' ),
   );  
   $args = array(      
       'labels'            => $labels,
       'hierarchical'      => true,
   );
   register_taxonomy( 'Crop-cat', 'crop', $args );
}
...
All Crops Post
*------------------------------------------*/
add_action('init', 'home_hicrop');
function home_hicrop()
{
    $labels = array(
                    'name'              => _x('Hindi Crop', 'post type general name'),
                    'singular_name'     => _x('Hindi Crop', 'post type singular name'),
                    'add_new'           => _x('Add New', 'Hindi Crop'),
                    'add_new_item'      => __('Add New Hindi Crop'),
                    'edit_item'         => __('Edit Hindi Crop'),
                    'new_item'          => __('New Hindi Crop'),
                    'all_items'         => __('All Hindi Crops'),
                    'view_item'         => __('View Hindi Crop'),
                    'search_items'      => __('Search Hindi Crop'),
                    'not_found'         =>  __('No Hindi Crop found'),
                    'not_found_in_trash'=> __('No Hindi Crop found in Trash'),
                    'parent_item_colon' => '',
                    'menu_name'         => 'Hindi Crops'
                  );
    $args = array(
                    'labels'                => $labels,
                    'public'                => true,
...
register_post_type('hindi-crop',$args);
flush_rewrite_rules();
};

/*-----------------   Crop Post Wp Rest API Data -------------------------*/


function create_crop_post_type() {
    register_post_type('crop',
        array(
            'labels' => array(
                'name' => __('Crops'),
...
add_action('init', 'create_crop_post_type');



function custom_crop_rest_route() {
    register_rest_route('custom/v1', '/crops', array(
        'methods' => 'GET',
        'callback' => 'get_custom_crops',
        'args' => array(
            'id' => array(
                'required' => false,
...
add_action('rest_api_init', 'custom_crop_rest_route');

function get_custom_crops(WP_REST_Request $request) {
   $id = $request->get_param('id');
    $title = $request->get_param('title');
if ($id) {
        // If the ID parameter is provided, return the post data along with ACF fields for that specific post
        $post = get_post($id);
        ...
        if ($post) {
            ...
            $response = array(
                'status' => 'true',
                'message' => 'Post data with ACF fields retrieved successfully',
                'data' => $post_data,
            );
            ...
            $response = array(
                'status' => 'false',
                'message' => 'No post found for the given ID',
                'data' => array(),
            );
    ...
    // Custom query to fetch crops based on title parameter
     $args = array(
            'post_type' => 'crop',
        );

        if ($title) {
            $args['s'] = $title; // Search by post title
        }
        ...
        if ($query->have_posts()) {
            ...
            while ($query->have_posts()) {
                ...
                $post_data = array(
                    ...
                    'image_url' => get_the_post_thumbnail_url($post_id, 'full'), // Get the featured image URL
                );
...
/*----------Hindi Crop Product wp rest API ------------------  */

function create_hindi_crop_post_type() {
    register_post_type('hindi-crop',
        array(
            'labels' => array(
                'name' => __('Hindi Crops'),
                'singular_name' => __('Hindi Crop')
            ),
            'public' => true,
            'has_archive' => true,
            'show_in_rest' => true, // Enable REST API support
...
add_action('init', 'create_hindi_crop_post_type');

function custom_hindi_crop_rest_route() {
    register_rest_route('custom/v1', '/hindi-crops', array(
        'methods' => 'GET',
        'callback' => 'get_custom_hindi_crops',
        'args' => array(
            'id' => array(
                'required' => false,
                'validate_callback' => function($param, $request, $key) {
...
add_action('rest_api_init', 'custom_hindi_crop_rest_route');

function get_custom_hindi_crops(WP_REST_Request $request) {
    $id = $request->get_param('id');
    $title = $request->get_param('title');
    ...
    if ($id) {
        ...
        // Custom query to fetch hindi-crop posts based on parameters
        $args = array(
            'post_type' => 'hindi-crop',
        );
